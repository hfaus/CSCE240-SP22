...
/** STREAM
 * input = (command line, cin); (file, ifstream)
 * output = (command line, cout); (file, ofstream)
 * both ^ fstream, sstream
 * steps: open, operation (read, write, both), close
 * 
 * open(filename, mode)
 * modes:
 ...
 *  ate = set initial location at the end of file, if flag not set it is at beginnng of file
 *  app = all output operations are done at the end of file, appending the content to the current content of the file
 *  trunc = if the file is opended for operations that already exist, its previous content is deleted and replaced
 **/

/** ERROR HANDLING
 * bad() = true if reading or writing fails
 * fail() = true same cases as bad() but format as well
 * eof() = true if open file has reached the end
 * good() = false if any above are true
 **/
