...
void read() {
    ...
    string personal;  // paragraph of personal information
    string jobHistory, residence, repParents, college, family, birthday, unknownPersonal, committee;  // for the different personal info parts
                 ...
                 if (ph1 == "Information") {  // to check that it is "Personal Information"
                     ...
                     while (p2 != "Committee") {  // will keep going until the "Committee" portion
                         if (p2 == "-") {  // go down a line when bulletpoint is found
                            if (strcasestr(personal.c_str(),"residing")) {
                                residence = personal;
                            } else if (strcasestr(personal.c_str(),"born")) {
                                birthday = personal;
                            } else if ((strcasestr(personal.c_str(),"daughter")) || strcasestr(personal.c_str(),"son")) {
                                repParents = personal;
                            } else if ((strcasestr(personal.c_str(),"college")) || strcasestr(personal.c_str(),"university")) {
                                college = personal;
                            } else if ((strcasestr(personal.c_str(),"married")) || strcasestr(personal.c_str(),"divorced") || strcasestr(personal.c_str(),"children")) {
                                family = personal;
                            } else if ((strcasestr(personal.c_str(),"assistant")) || strcasestr(personal.c_str(),"administrator") || strcasestr(personal.c_str(),"member")) {
                                jobHistory += personal + " ";
                            } else {
                                unknownPersonal += personal + " ";
                            }
                        personal.clear();
                        in_myfile >> p2;
                         }  else {
    ...
    if (out_myfile.is_open()) {
        out_myfile << "hi hey hello: Hello! What would you like to know?" << endl;
        out_myfile << "everything what is the first last nick name: The representative's name is " << firstName << " " << nickName << " "<< lastName << endl;
        out_myfile << "everything what is the political party affiliation: The representative is a " << party << endl;
        out_myfile << "everything what is the district support number: The representative's district number is " << districtNum << endl;
        out_myfile << "everything what is the district support county: The representative's district county is " << districtCounty << endl;
        out_myfile << "everything what is the contact where live home address: The representative's home address is " << homeAdd << endl;
        out_myfile << "everything what is the contact where work columbia address: The representative's work address is " << colaAdd << endl;
        out_myfile << "everything what is the contact how do reach home cell phone number: The representative's cell phone number is " << cellPhone << endl;
        out_myfile << "everything what is the contact how do reach work business phone number: The representative's business phone number is " << busPhone << endl;
        out_myfile << "everything is the personal about job history: " << jobHistory << endl;
        out_myfile << "everything is the personal about where live: " << residence << endl;
        out_myfile << "everything is the personal about who related parent family: " << repParents << endl;
        out_myfile << "everything is the personal about what where college school university: " << college << endl;
        out_myfile << "everything is the personal about who related husband wife marry married spouse kid child family: " << family << endl;
        out_myfile << "everything is the personal about birthday born: " << birthday << endl;
        out_myfile << "everything is the personal about: " << unknownPersonal << endl;
        out_myfile << "everything rep representative what is the committee on: The representative is on the " << committee << " committee" << endl;
    } else {
...
void questionMatch(string inputInfo, double basePercent) {
    string input_filename = "data/output.txt";  // name of file to be parsed
...
void createSession() {
        ...
        if(in_myfile) {
            ...
            ofstream newSession (newFileName);
             if (newSession.is_open()) {
                // looping through chat vector and adding info to new chat session .txt file
    ...
    // outputting the statistics to .csv file
...
void sessionSearch(int ssInput, int iNumber) {
    ...
    string sSession, sAsks, sResponses, sDuration;  // string version
    int iSession, iAsks, iResponses, iDuration;  // int version
    // initializing to 0
    int tSession = 0;
    int tAsks = 0;
    int tResponses = 0;
    ...
    if (in_myfile_csv.is_open()) {
        while (getline(in_myfile_csv, line)) {
            ...
            // turning string variables to ints
            ...
            tDuration += iDuration;
            // if the session is the session the user asked for
            if (iSession == iNumber) {
        ...
        if (ssInput == 1) {  // summary and showchat
            if (one_output == "") {  // if there is an invalid input
        ...
        } else if (ssInput == 2) {  // just summary
        ...
        } else if (ssInput == 3) {  // just showchat
            fstream in_myfile_session (input_filename_session);
            if (in_myfile_session.is_open()) {  // valid input
                cout << "Chat " << iNumber << " chat is:" << endl;
                // going through text file to get all of it's information
...
void commandMatch(string inputInfo) {
    ...
    if ((numSummary == 1) && (numShowchat == 1)) {  // summary and showchat
        ssInput = 1;
        sessionSearch(ssInput, iNumber);
    } else if ((numSummary == 1) && (numShowchat == 0)) {  // only summary
        ssInput = 2;
        iNumber = 0;
        sessionSearch(ssInput, iNumber);
    } else if ((numSummary == 0) && (numShowchat == 1)) {  // only showchat
        ssInput = 3;
        sessionSearch(ssInput, iNumber);
    } else {
...
int main(int argc, char* argv[]) {
    ...
    time(&start);
        ...
        while (inputInfo != "Quit" && inputInfo != "q" && inputInfo != "quit") {
            ...
            if(strcasestr(inputInfo.c_str(),"-")) {  // if command is asked
                commandMatch(inputInfo);
            } else {  // if regular question askes
        ...
        // ending the time and calculating total time taken
        time(&end);
